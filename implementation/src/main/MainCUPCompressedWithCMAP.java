...
import cup.CUPAlgorithmCompressed;
import data.ClickstreamSequenceDatabase;
import java.io.IOException;
import reporting.ResearchOutputWriter;

/**
 * Main Entry Point for CUP+DUB with CMAP pruning enabled.
 */
public class MainCUPCompressedWithCMAP {
    ...
    private static boolean noTrace = true; // default to true for benchmark efficiency
    private static boolean savePatterns = false;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("--data") && i + 1 < args.length) {
                ...
                noTrace = false;
            } else if (args[i].equals("--save-patterns")) {
                savePatterns = true;
            }
        }
    ...
    private static String resolveOutputDir(String customOutputDir, String datasetName) {
        return customOutputDir != null ? customOutputDir : OUTPUT_ROOT + "/" + datasetName + "_compressed";
    }

    private static void runDataset(String dataFile, String outputDir, double minSupport) {
        System.out.println("=========================================================");
        System.out.println("CUP-Compressed Algorithm with CMAP (Paper 1 experiment variant)");
        System.out.println("=========================================================");
        ...
        try {
            ...
            CUPAlgorithmCompressed algo = new CUPAlgorithmCompressed(minSupport, logger);
            algo.setStorePatternsInMemory(false);
            algo.setWritePatternsToFile(savePatterns);
            algo.setUseCmap(true);
            algo.run(db);
        } catch (IOException e) {
